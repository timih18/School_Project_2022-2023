    ...
    if call.message:
        if call.from_user.id == data['admin_id']:
            if call.data == 'admin':
                ...
                msg = bot.send_message(call.message.chat.id, 'Введи через "/" номер растения, которое хочешь удалить.')
                bot.register_next_step_handler(msg, after_delete)
        else:
...
def after_delete(message):
    global data
    elem = message.text[1:]
    for i in range(int(elem), data['cnt_plants']):
        data[str(i)] = data[str(i+1)]
    data.pop(str(data['cnt_plants']))
    data['cnt_plants'] -= 1
